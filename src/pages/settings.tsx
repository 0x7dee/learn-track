...
>>>>>>> parent of 116130b (minor bug fixes)
  ...
  const validateMemberNumber = async (number: string) => {
    ...
    let result = await postData("https://api.gumroad.com/v2/licenses/verify", {
      product_id: "HcIl1HHJ_XBLD9_kRDS-tw==",
      license_key: number,
    });
    if (result.success) {
  ...
  const importData = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (parsedFile.links)
      await chrome.storage.local.set({ links: parsedFile.links });
    if (parsedFile.viewHistory)
      await chrome.storage.local.set({ viewHistory: parsedFile.viewHistory });
  };
          ...
          <div className="absolute w-48 top-3 -left-3/4 bg-white z-10 hidden peer-hover:block overflow-hidden px-4 py-2 rounded-md border-slate-100 border-2">
            <p>
              <span className="text-red-400">
                Caution: importing data will override your existing application
                data, this cannot be overridden.
              </span>
            </p>
          </div>
        </div>
      ...
      {showDeletePopup ? (
        <div className="rounded">
          <p className="text-sm mb-4">
            Are you sure? Once deleted, your data cannot be recovered.
          </p>
          <div className="flex flex-row">
            ...
            <button
              className="rounded-md text-xs py-2 px-0 w-full border text-sky-400 border-sky-400 hover:text-neutral-100 hover:bg-sky-400 transition ease-in-out duration-300"
        ...
        <>
          <button
            className="rounded-md text-xs py-2 px-0 w-full border text-red-400 border-red-400 hover:text-neutral-100 hover:bg-red-400 transition ease-in-out duration-300"
